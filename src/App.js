...
const DB_EVENTS = [
  ...
  {
    id: 'pass-4-days', title: 'Eutherieste Festival - Presale (4 Days Pass)', date: '15 - 18 Juni 2026', venue: EVENT_VENUE, image: EVENT_IMAGE_URL, priceStart: 17000000,
    packages: [
      { id: '4d-vvip', name: '4 Days (All Lineups) - VVIP', price: 39000000, desc: b_vvip, dateStr: '15-18 Juni 2026' },
      { id: '4d-vip', name: '4 Days (All Lineups) - VIP', price: 36000000, desc: b_vip, dateStr: '15-18 Juni 2026' },
      { id: '4d-plat', name: '4 Days (All Lineups) - PLATINUM', price: 32000000, desc: b_plat, dateStr: '15-18 Juni 2026' },
      { id: '4d-c1', name: '4 Days (All Lineups) - CAT 1', price: 24000000, desc: b_c1, dateStr: '15-18 Juni 2026' },
      { id: '4d-c2', name: '4 Days (All Lineups) - CAT 2', price: 17000000, desc: b_c2, dateStr: '15-18 Juni 2026' }
    ]
  }
];
...
export default function App() {
  ...
  const handleGoToCheckout = () => {
    ...
    // Inisialisasi pengunjung sejumlah tiket
  ...
  // Proses sukses pembayaran dan simpan ke Local Storage
  const processPaymentSuccess = () => {
    ...
    setTimeout(() => {
      ...
      // Gabungkan dengan tiket lama dan urutkan
  ...
  // Digunakan langsung di return agar tidak terjadi re-mount komponen (solusi glitch animasi)
  ...
  const renderSplash = () => (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-5 animate-[popIn_0.4s_ease-out] w-full">
      <div className="text-center mb-12 relative flex flex-col items-center">
        ...
        {/* LOGO CUSTOM - Tanpa Bingkai dan Background */}
      ...
      <button 
        onClick={() => navTo('home')} 
        className="w-full max-w-sm py-4 rounded-2xl font-bold text-lg text-white bg-gradient-to-r from-red-600 to-orange-500 shadow-[0_10px_25px_rgba(249,115,22,0.4)] active:scale-95 transition-transform"
      >
        Get Started
      </button>
    </div>
  ...
  const renderHome = () => (
    <div className="pb-28 animate-[fadeSlideUp_0.4s_ease-out] w-full max-w-3xl mx-auto">
      <div className="bg-gradient-to-br from-red-600 to-orange-500 px-5 pt-10 pb-12 text-white rounded-b-[2rem] shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold mb-2 leading-tight">Eutheriestians!</h1>
          <p className="text-sm md:text-base text-white/90 font-medium bg-white/20 inline-block px-4 py-2 rounded-lg backdrop-blur-sm border border-white/20 mt-2">
            Udah siap belum buat nonton Eutherieste Festival? ✨
          </p>
        </div>
      </div>

      <div className="p-5 md:p-8 -mt-6 relative z-20">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg md:text-xl font-bold text-gray-800">Hot Updates</h2>
          </div>
          <div className="bg-orange-50 border border-orange-100 p-5 rounded-xl">
            ...
            <h3 className="font-bold text-gray-900 mt-3 text-lg">Coldplay, Ariana, The Weeknd & Bruno Mars!</h3>
        ...
        <h2 className="text-lg md:text-xl font-bold text-gray-800 mb-5 px-1">Cara Pemesanan Tiket</h2>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-center gap-5 bg-white p-5 rounded-2xl shadow-sm border border-gray-50">
              <div className={`${step.c} w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-lg`}>{i + 1}</div>
              <p className="text-sm md:text-base text-gray-600 font-medium">{step.t}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderEvents = () => (
    <div className="pb-28 min-h-screen w-full animate-[fadeSlideUp_0.4s_ease-out] bg-gray-50">
      <div className="bg-white p-5 md:py-6 md:px-8 sticky top-0 z-10 shadow-sm">
        <div className="max-w-3xl mx-auto">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">Eutherieste Events</h1>
        ...
        {DB_EVENTS.map(ev => (
          <div key={ev.id} onClick={() => handleOpenEventDetail(ev)} className="bg-white rounded-[1.5rem] shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden border border-gray-100 cursor-pointer active:scale-[0.98] transition-transform">
            <div className="relative">
              {/* EVENT IMAGE - Memaksa format 16:9 di semua layar */}
            ...
            <div className="p-5 md:p-6">
              <h2 className="font-extrabold text-gray-900 text-lg md:text-xl mb-2 leading-tight">{ev.title}</h2>
              <p className="text-sm text-gray-500 font-medium mb-5 flex items-center"><Calendar className="w-4 h-4 mr-2" /> {ev.date}</p>
  ...
  const renderDetail = () => {
    ...
    return (
      <div className="bg-gray-50 min-h-screen relative pb-32 animate-[fadeSlideUp_0.4s_ease-out] w-full">
        <div className="absolute top-4 md:top-6 left-0 z-20 flex gap-2 w-full px-4 md:px-8 justify-between max-w-4xl mx-auto right-0">
          <button onClick={() => navTo('events')} className="bg-white/90 backdrop-blur w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform">
            <ChevronLeft className="text-gray-800 w-5 h-5 md:w-6 md:h-6" />
          </button>
          <div className="flex gap-3">
            <button onClick={() => showToast('Disimpan ke Favorite')} className="bg-white/90 backdrop-blur w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform">
              <Heart className="text-gray-600 w-5 h-5 md:w-6 md:h-6" />
            </button>
            <button onClick={() => showToast('Tautan event disalin')} className="bg-white/90 backdrop-blur w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform">
        ...
        <div className="relative max-w-4xl mx-auto">
          <img src={selectedEvent.image} alt="Cover" className="w-full aspect-video md:aspect-[21/9] object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
        </div>
        
        <div className="bg-white p-6 md:p-10 -mt-8 relative z-10 rounded-t-3xl shadow-sm max-w-4xl mx-auto">
          ...
          <div className="space-y-4 mb-6 bg-gray-50 p-5 rounded-2xl border border-gray-100 flex flex-col md:flex-row md:space-y-0 md:gap-8">
            <div className="flex items-center gap-4 flex-1">
              <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center"><MapPin className="text-red-500 w-6 h-6"/></div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase">Lokasi</p>
                <p className="text-sm md:text-base font-bold text-gray-800">{selectedEvent.venue}</p>
              </div>
            </div>
            <div className="w-full h-[1px] md:w-[1px] md:h-12 bg-gray-200"></div>
            <div className="flex items-center gap-4 flex-1">
              <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center"><Calendar className="text-orange-500 w-6 h-6"/></div>
              <div>
                <p className="text-xs text-gray-500 font-semibold uppercase">Tanggal</p>
                <p className="text-sm md:text-base font-bold text-gray-800">{selectedEvent.date}</p>
              </div>
            </div>
          </div>
        </div>

        <div className="mt-2 bg-white p-6 md:p-10 pb-12 shadow-sm max-w-4xl mx-auto rounded-b-3xl">
          <h2 className="text-xl md:text-2xl font-bold mb-6 text-gray-900">Pilih Tiket & Kategori</h2>
          <div className="space-y-4">
            {selectedEvent.packages.map(pkg => (
              <div key={pkg.id} onClick={() => handleOpenQtyModal(pkg)} className="border border-gray-200 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-4 bg-white hover:border-orange-300 transition-colors cursor-pointer active:scale-[0.99] shadow-sm">
  ...
  const renderCheckout = () => {
    ...
    return (
      <div className="bg-gray-50 min-h-screen pb-48 animate-[fadeSlideUp_0.4s_ease-out] w-full">
        <div className="bg-white p-4 md:px-8 md:py-5 sticky top-0 z-20 shadow-sm flex items-center gap-4">
          <button onClick={() => navTo('detail')} className="text-gray-800 bg-gray-100 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center active:scale-95 transition-transform"><ChevronLeft className="w-5 h-5 md:w-6 md:h-6"/></button>
          <h1 className="font-bold text-lg md:text-xl text-gray-900">Selesaikan Pesanan</h1>
        </div>

        <div className="p-5 md:p-8 space-y-6 md:space-y-8 max-w-3xl mx-auto mt-2">
          {/* Ringkasan */}
          <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-orange-100 relative overflow-hidden">
            ...
            <h3 className="font-bold text-gray-900 mt-4 mb-1 text-base md:text-lg leading-tight relative z-10">{selectedEvent.title}</h3>
            <p className="text-gray-500 text-sm mb-4 relative z-10">{selectedPackage.dateStr || selectedEvent.date}</p>
            
            <div className="border-t border-dashed border-gray-200 pt-4 flex flex-col gap-3 relative z-10">
              <div className="flex justify-between items-end">
                <div>
                  <p className="font-bold text-base md:text-lg bg-gradient-to-r from-red-600 to-orange-500 bg-clip-text text-transparent">{selectedPackage.name}</p>
                  <p className="text-sm text-gray-500 font-medium mt-1 flex items-center"><Ticket className="w-4 h-4 mr-1"/> {quantity} Tiket</p>
                </div>
                <p className="font-bold text-gray-900 text-lg">{formatRupiah(baseTotal)}</p>
              </div>
              <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg mt-1">
                <p className="text-sm text-gray-500 font-medium">Pajak (11%)</p>
                <p className="text-sm font-bold text-gray-700">{formatRupiah(tax)}</p>
              </div>
              <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg -mt-1">
                <p className="text-sm text-gray-500 font-medium">Booking Fee</p>
                <p className="text-sm font-bold text-gray-700">{formatRupiah(fee)}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="font-bold text-gray-900 md:text-lg">Detail Pemesan</h2>
            </div>
            <div className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6 md:gap-8">
          ...
          <div>
            {visitorDetails.map((v, i) => (
              <div key={i} className="mt-6 md:mt-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="font-bold text-gray-900 md:text-lg">Pengunjung {i + 1}</h2>
                </div>
                <div className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-5 md:gap-6">
                  ...
                  {!v.samaDenganPemesan && (
                    <div className="animate-[fadeSlideUp_0.2s_ease-out] flex flex-col gap-6">
                      <div>
                        <label className="text-xs md:text-sm font-bold text-gray-500 uppercase tracking-wide block mb-2">Nama Pengunjung</label>
                        <input type="text" value={v.nama} onChange={(e) => handleVisitorChange(i, 'nama', e.target.value)} className="w-full border-b-2 border-gray-100 py-2.5 outline-none focus:border-orange-500 text-base font-bold text-gray-800 bg-transparent" placeholder="Nama sesuai ID" />
                      </div>
                      <div>
                        <label className="text-xs md:text-sm font-bold text-gray-500 uppercase tracking-wide block mb-2">NIK Pengunjung</label>
                        <input type="number" value={v.nik} onChange={(e) => handleVisitorChange(i, 'nik', e.target.value)} className="w-full border-b-2 border-gray-100 py-2.5 outline-none focus:border-orange-500 text-base font-bold text-gray-800 bg-transparent" placeholder="16 Digit NIK" />
                      </div>
                    </div>
                  )}
                </div>
        ...
        {/* Sticky Bottom Checkout dipisah dari urutan form agar selalu di bawah */}
        <div className="fixed bottom-0 left-0 w-full bg-white p-5 border-t border-gray-200 z-50 flex justify-between items-center shadow-[0_-10px_20px_rgba(0,0,0,0.08)] px-5 md:px-8">
          <div className="max-w-3xl mx-auto w-full flex justify-between items-center">
            ...
            <button onClick={handleValidateAndPay} className="bg-gradient-to-r from-red-600 to-orange-500 text-white px-8 py-3.5 md:py-4 rounded-xl md:rounded-2xl font-bold text-base md:text-lg active:scale-95 shadow-lg flex items-center transition-transform">
  ...
  const renderPayment = () => (
    <div className="bg-gray-50 min-h-screen animate-[fadeSlideUp_0.4s_ease-out] relative w-full">
      <div className="bg-white p-4 md:py-5 sticky top-0 z-10 shadow-sm flex items-center justify-center border-b border-gray-100">
        <h1 className="font-bold text-lg md:text-xl text-gray-900">Pembayaran QRIS</h1>
      </div>
      
      <div className="p-6 md:p-10 flex flex-col items-center text-center mt-4 md:mt-8">
        <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-lg border border-gray-100 w-full max-w-md flex flex-col items-center relative overflow-hidden">
          ...
          <h2 className="font-bold text-gray-900 text-xl mb-2">Scan untuk Membayar</h2>
          ...
          <div className="p-4 border border-gray-200 rounded-3xl mb-10 bg-white shadow-inner relative">
            ...
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=EUTHERIESTE-${Date.now()}`} alt="QR Code" className="w-56 h-56 md:w-64 md:h-64" />
          </div>
        ...
        <p className="text-sm text-gray-400 mt-10 font-medium">Harap jangan tutup halaman ini selama proses pembayaran berlangsung.</p>
      </div>
      ...
      {isPaymentLoading && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-md z-50 flex flex-col items-center justify-center">
          ...
          <h3 className="font-extrabold text-2xl text-gray-900 mb-3">Memproses...</h3>
          <p className="font-medium text-gray-500 text-base">Menunggu verifikasi pembayaran</p>
        </div>
      )}
    </div>
  );
  ...
  const renderTickets = () => (
    <div className="bg-gray-50 min-h-screen pb-28 animate-[fadeSlideUp_0.4s_ease-out] w-full">
      <div className="bg-white p-5 md:py-6 md:px-8 sticky top-0 z-10 shadow-sm border-b border-gray-100">
        <h1 className="font-extrabold text-2xl md:text-3xl text-gray-900 max-w-3xl mx-auto">Tiket Saya</h1>
      </div>
      ...
      <div className="p-5 md:p-8 max-w-3xl mx-auto">
        {myTickets.length === 0 ? (
          <div className="text-center py-32">
            ...
            <button onClick={() => navTo('events')} className="mt-8 px-10 py-3.5 border-2 border-orange-500 text-orange-600 rounded-full font-bold active:scale-95 text-lg transition-transform hover:bg-orange-50">Cari Event</button>
          </div>
        ) : (
          <div className="space-y-6">
            {myTickets.map((ticket, i) => (
              <div key={i} onClick={() => { setViewTicketData(ticket); navTo('ticket-detail'); }} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col gap-4 cursor-pointer active:scale-[0.98] transition-transform">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[10px] md:text-xs text-gray-400 font-mono">{ticket.id}</span>
                </div>
                <div className="flex gap-4 md:gap-6 border-b border-dashed border-gray-200 pb-5">
                  <img src={ticket.image} className="w-24 h-20 md:w-32 md:h-24 rounded-xl object-cover shadow-sm bg-gray-100" alt="Cover" />
                  <div className="flex-1">
                    <h3 className="font-extrabold text-base md:text-lg text-gray-900 leading-tight mb-2 line-clamp-2">{ticket.eventTitle}</h3>
                    <p className="text-[10px] md:text-xs text-gray-500 font-medium flex items-center"><Calendar className="w-3.5 h-3.5 mr-1.5"/> {ticket.date}</p>
                  </div>
                </div>
  ...
  const renderTicketDetail = () => {
    ...
    return (
      <div className="bg-gray-900 min-h-screen relative pb-10 animate-[fadeSlideUp_0.4s_ease-out] overflow-y-auto w-full">
        <div className="p-5 md:px-8 md:py-6 sticky top-0 z-20 flex items-center gap-4 text-white max-w-2xl mx-auto">
          <button onClick={() => navTo('tickets')} className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/10 flex items-center justify-center active:scale-95 transition-transform"><X className="w-5 h-5 md:w-6 md:h-6"/></button>
          <h1 className="font-bold text-lg md:text-xl">E-Ticket</h1>
        </div>

        <div className="px-6 py-4 max-w-2xl mx-auto">
          <div className="bg-white rounded-3xl relative filter drop-shadow-xl overflow-hidden mb-8">
            ...
            <div className="relative border-t-2 border-dashed border-gray-200 mx-6 md:mx-8 my-2">
               <div className="absolute -top-4 -left-10 w-8 h-8 bg-gray-900 rounded-full shadow-inner"></div>
               <div className="absolute -top-4 -right-10 w-8 h-8 bg-gray-900 rounded-full shadow-inner"></div>
            </div>

            <div className="p-6 md:p-8 pt-4 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">Tunjukkan QR ini saat penukaran tiket</p>
              <div className="inline-block border-2 border-gray-100 p-3 rounded-2xl mb-4 bg-white">
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${t.id}`} alt="QR" className="w-48 h-48 md:w-56 md:h-56" />
              </div>
              <p className="text-sm font-mono text-gray-400 tracking-widest">{t.id}</p>
            </div>
          </div>
          
          <button className="w-full bg-white/10 hover:bg-white/20 text-white border border-white/20 py-4 md:py-5 rounded-2xl font-bold text-lg flex justify-center items-center gap-3 active:scale-95 transition-all">
  ...
  // --- RENDER MAIN LAYOUT ---
  return (
    <div className="min-h-screen bg-gray-50 font-sans w-full relative">
      ...
      {['home', 'events', 'tickets'].includes(currentView) && (
        <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 flex justify-around p-2 pb-6 md:pb-4 z-40 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
          <div className="max-w-md w-full flex justify-around mx-auto">
            {[
              ...
              <div 
                ...
                className={`flex flex-col items-center gap-1 cursor-pointer p-3 rounded-2xl transition-colors w-24 ${currentView === nav.id ? 'text-orange-500 bg-orange-50' : 'text-gray-400 hover:bg-gray-50'}`}
              >
                <nav.icon className="w-6 h-6 md:w-7 md:h-7 mb-1" />
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-wide">{nav.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {isQtyModalOpen && selectedPackage && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="bg-white w-full rounded-t-[2rem] p-6 md:p-10 pb-8 relative z-10 animate-[slideUp_0.3s_ease-out] shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
            <div className="max-w-2xl mx-auto">
              <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-8"></div>
              <h3 className="font-bold text-lg md:text-xl text-gray-900 mb-2">{selectedPackage.name}</h3>
              <p className="bg-gradient-to-r from-red-600 to-orange-500 bg-clip-text text-transparent font-bold text-2xl mb-8">{formatRupiah(selectedPackage.price)} / pax</p>
              ...
              <div className="flex justify-between items-center bg-gray-50 p-5 rounded-2xl border border-gray-100 mb-8">
                ...
                <div className="flex items-center gap-5">
                  <button onClick={() => handleUpdateQty(-1)} className="w-12 h-12 rounded-full bg-white border border-gray-200 shadow-sm text-gray-600 flex items-center justify-center active:scale-95"><Minus className="w-5 h-5"/></button>
                  <span className="font-extrabold text-2xl w-6 text-center">{quantity}</span>
                  <button onClick={() => handleUpdateQty(1)} className="w-12 h-12 rounded-full bg-gradient-to-r from-red-600 to-orange-500 text-white shadow-md flex items-center justify-center active:scale-95"><Plus className="w-5 h-5"/></button>
                </div>
              </div>
              ...
              <button onClick={handleGoToCheckout} className="w-full bg-gradient-to-r from-red-600 to-orange-500 text-white py-4 md:py-5 rounded-2xl font-bold text-lg md:text-xl shadow-[0_8px_20px_rgba(249,115,22,0.3)] active:scale-95 mt-4 transition-transform">
                Pesan Sekarang
              </button>
            </div>
          </div>
      ...
      {/* SUCCESS MODAL (Design TIX ID Clean Style) */}
      {isSuccessModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-5">
          <div className="bg-white rounded-[2rem] pt-12 pb-8 px-6 w-full max-w-sm text-center animate-[popIn_0.3s_ease-out] shadow-2xl relative">
            {/* Aksen Hijau di Atas */}
            <div className="absolute top-0 left-0 w-full h-4 bg-green-500 rounded-t-[2rem]"></div>
            
            {/* Lingkaran Checkmark (Overlap ke atas batas modal) */}
            ...
            <h2 className="text-2xl font-extrabold text-gray-900 mb-3 mt-4 tracking-tight">Pembayaran Berhasil!</h2>
